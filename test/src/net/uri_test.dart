  ...
  group("class:uri", () {
    ...
    group("new URI.tag()", () {
      ...
      void doTestNewUriTagWithDomainName(final String expected, final String authority,  final DateTime date,
        ...
        final URI expectedUri = URI.parser.parseValue(expected);
        final URI tag = new URI.tag(
            authorityDomain: DomainName.parser.parseValue(authority),
            date: date,
            specificPath: Path.parser.parseValue(specificPath),
            query: query,
            fragment: fragment);
        test(expected, () =>
    ...
    group("canonicalize()", () {
      void doTestCanonicalize(final String expected, final String testUri) {
        test("with $testUri", () =>
            expect(URI.parser.parse(testUri).value.canonicalize().toString(), equals(expected)));
      }
      ...
      test("Uri without scheme", () =>
          expect(URI.parser.parse("www.example.com").value.canonicalize().toString(), equals("www.example.com")));
    });
    ...
    group("static method: relativeReference()", () {
      void doTestRelativeReference(final String expected, final String base, final String relative) {
        test("with base:$base and relative:$relative", () {
          final URI baseUri = URI.parser.parse(base).value;
          final URI relativeUri = URI.parser.parse(relative).value;
          final URI expectedUri = URI.parser.parse(expected).value;

...
Parser<IAuthority> _authorityParser(
  ...
  final Parser<String> optionalUserInfo =
      (userInfo + isChar("@")).map((final Pair<String, int> e) =>
          e.e0).orElse("");
  final Parser<Option<int>> optionalPort =
      (isChar(":") + _PORT).map((final Pair<int, int> e) =>
          e.e1).optional();

  return (optionalUserInfo + host + optionalPort)
      .map((final Tuple3<String, Either<InternationalDomainName, IPAddress>, Option<int>> e) =>
          builder(e.e0, e.e1, e.e2));
}
